...
export default function RateCard({
	...
	isRateSelected: (rate: AvailableRate) => boolean;
	isFoodPlanSelected: (foodPlan: Partial<FoodPlanSelect>) => boolean;
}) {
							...
							<div className="flex flex-col items-start justify-center w-full gap-2 px-4 py-3 rounded-t-md bg-primary/10">
								...
								<div className="flex items-center justify-between w-full">
									...
									<span className="font-bold text-accent">
										₹
										{(rate.rate ?? 0) + foodPlan.tariff * (rate.headCount || 1)}
										/-
									</span>
								</div>
							</div>
          ...
          <span className="font-bold material-symbols-outlined">remove</span>
							...
							{isRateSelected(rate) ? (
								...
								<button
									...
									onClick={() => onRateSelected(rate, foodPlan)}
								>
									Select
								</button>
							)}
