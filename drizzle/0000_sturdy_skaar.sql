...
CREATE TABLE `Availability` (
	...
	`stayDate` integer NOT NULL,
	`avlCount` integer DEFAULT 1 NOT NULL,
...
CREATE TABLE `Room` (
	...
	`roomCount` integer DEFAULT 1 NOT NULL,
...
CREATE TABLE `RoomReserved` (
	...
	FOREIGN KEY (`reservationId`) REFERENCES `Reservation`(`id`) ON UPDATE cascade ON DELETE cascade,
...
CREATE UNIQUE INDEX `Availability_stayDate_roomId_key` ON `Availability` (`stayDate`,`roomId`);--> statement-breakpoint
CREATE INDEX `Availability_roomId_idx` ON `Availability` (`roomId`);--> statement-breakpoint
...
CREATE INDEX `HomestayAmenities_homestayId_idx` ON `FoodPlan` (`homestayId`);--> statement-breakpoint
...
CREATE INDEX `Room_categoryId_idx` ON `Room` (`categoryId`);--> statement-breakpoint
...
CREATE INDEX `RoomReserved_roomId_idx` ON `RoomReserved` (`rateId`);--> statement-breakpoint
CREATE INDEX `RoomReserved_foodPlanId_idx` ON `RoomReserved` (`foodPlanId`);--> statement-breakpoint
CREATE INDEX `RoomReserved_reservationId_idx` ON `RoomReserved` (`reservationId`);--> statement-breakpoint
