...
export type FoodPlanSelect = InferSelectModel<typeof foodPlan>;
...
export type RoomSelect = InferSelectModel<typeof room>;
...
export const availability = sqliteTable(
	"Availability",
	{
		...
		rate: integer("rate").notNull().default(999),
		rateId: text("rateId")
			.notNull()
			.references(() => rate.id, { onDelete: "cascade", onUpdate: "cascade" }),
	},
	(table) => ({
		stayDateRoomKey: uniqueIndex("Availability_stayDate_rateId_key").on(
			table.stayDate,
			table.rateId,
		),
		rateIdIdx: index("Availability_rateId_idx").on(table.rateId),
	}),
);

export const ratesToAvailability = sqliteTable(
	"RatesToAvailability",
	{
		rateId: text("rateId")
			.notNull()
			.references(() => rate.id, { onDelete: "cascade", onUpdate: "cascade" }),
		availabilityId: integer("availabilityId")
			.notNull()
	...
	(table) => {
		return {
			pk: primaryKey({
				columns: [table.rateId, table.availabilityId],
			}),
		};
	},
);
...
export const availabilityRelations = relations(availability, ({ one }) => ({
	rateData: one(rate, {
		fields: [availability.rateId],
		references: [rate.id],
	}),
}));

export const ratesToAvailabilityRelations = relations(
	ratesToAvailability,
	({ one }) => ({
		availability: one(availability, {
			fields: [ratesToAvailability.availabilityId],
			references: [availability.id],
		}),
		rate: one(rate, {
			fields: [ratesToAvailability.rateId],
			references: [rate.id],
		}),
	}),
);
